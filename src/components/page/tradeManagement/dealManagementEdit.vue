          ...
          <Col span="20" offset="1">
            <Form label-position="right" :label-width="250" ref="formValidate" :model="formValidate" :rules="ruleValidate">
              ...
              <FormItem label="价格精度：" prop="priceAccuracy">
                <Input type="number" style="width:40%;min-width:300px;" v-model.number='formValidate.priceAccuracy'>
                  <span slot="append">位小数</span>
                </Input>
              </FormItem>
              <FormItem label="最小交易金额：" prop="minTransactionAmount">
                <Input type="number" style="width:40%;min-width:300px;" v-model.number='formValidate.minTransactionAmount'>
                  <span slot="append">AP</span>
                </Input>
              </FormItem>
              <FormItem label="最小交易数量：" prop="minTransactionNumber">
                <Input type="number" style="width:40%;min-width:300px;" v-model.number='formValidate.minTransactionNumber'></Input>
              </FormItem>
              <FormItem label="手续费：" prop="handlingFee">
                <Input type="number" style="width:40%;min-width:300px;" v-model.number='formValidate.handlingFee'>
                  <span slot="append">AP</span>
                </Input>
              </FormItem>
              <FormItem label="开盘价：" prop="openingPrice">
                <Input type="number" style="width:40%;min-width:300px;" v-model.number='formValidate.openingPrice'></Input>
              </FormItem>
            </Form>
          </Col>
        </Row>
...
<script>
  export default {
      ...
      return {
        ...
        ruleValidate: {
          copyrightId: [
           ...
           priceAccuracy: [
            { required: true, message: '请填写价格精度', trigger: 'blur',type:'number'}
          ],
           minTransactionAmount: [
            { required: true, message: '请填写单小交易金额', trigger: 'blur',type:'number'}
          ],
           minTransactionNumber: [
            { required: true, message: '请填写单小交易数量', trigger: 'blur',type:'number'}
          ],
           handlingFee: [
            { required: true, message: '请填写手续费', trigger: 'blur',type:'number' }
          ],
          openingPrice: [
            { required: true, message: '请填写开盘价', trigger: 'blur',type:'number'}
          ]
        }
      }
    },
    methods: {
      ...
      handleSubmit (name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            _this.$axios.post('/api/transactionCopyright/updateTransactionCopyright',form)
            .then(res=>{
      ...
      getShowData(){
        this.$axios.get('/api/transactionCopyright/getTransactionCopyrightById?id='+ this.formValidate.id)
        .then(res=>{
